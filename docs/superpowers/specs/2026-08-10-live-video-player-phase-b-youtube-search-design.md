...
**No-GPU fallback**: `jsmpeg.min.js` itself feature-detects WebGL at construction time — confirmed directly in its source (`this.renderer = !options.disableGl && WebGL.IsSupported() ? new WebGL(...) : new Canvas2D(...)`) — and falls back automatically to its own `Canvas2D` renderer when WebGL is unavailable (headless/software-only environments, some VMs, GPU-less machines). That fallback path still targets the same `<canvas>` element, still never touches `<video>`, and needs no option or code change on this project's side to activate — it is JSMpeg's default behavior. The one honest cost: on that fallback path the *render* step (not just decode) also runs on the CPU, so playback is more CPU-intensive on GPU-less hardware, though still functional — this is the same "GPU when available, CPU-only path still works" posture Phase A takes with its WebGPU→WebGL2 fallback (spec §12 there).
